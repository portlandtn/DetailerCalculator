...
namespace DetailerCalculator
{
    ...
    public partial class FRMDetailerCalculator : Form
    {
        ...
        {
            FunctionButtonClick("f2df");
            UserEntryBox.Focus();
        }

        private void DecimalToFootButton_Click(object sender, EventArgs e)
        {
            FunctionButtonClick("d2ff");
            UserEntryBox.Focus();
        }


        private void FootToDecimalFootButton_ChangeUICues(object sender, UICuesEventArgs e)
        {

        }


        private void DecimalInchToFootButton_Click(object sender, EventArgs e)
        {
            FunctionButtonClick("d2fi");
            UserEntryBox.Focus();
        }

        private void FootToDecimalInchButton_Click(object sender, EventArgs e)
        {
            FunctionButtonClick("f2di");
            UserEntryBox.Focus();
        }
        ...
        private void OutputWindowStringBuilder()
        {
            ...
            UndoButton.Enabled = (undoRedoStack.Count >= 2) ? true : false;
        }
        ...
        private void OutputWindowStringBuilder(decimal numberToAdd, int numbersToReplace)
        {
            ...
            while (numbersToReplace > 0)
            {
                ...
                {
                    MessageBox.Show("There are not enough numbers in the output window to perform the requested operation.", "Not Enough Operators Present", MessageBoxButtons.OK, MessageBoxIcon.Exclamation, MessageBoxDefaultButton.Button1, MessageBoxOptions.DefaultDesktopOnly);
                    return;
                }
            }
        ...
        private void FunctionButtonClick(string function)
        {
            ...
            if (function == "d2ff" || function == "f2df" || function == "d2fi" || function == "f2di")
            {
                var response = 0m;
                if (_OutputWindowList.Count > 0)
                {
                    switch (function)
                    {
                        case "d2ff":
                            response = Conversions.DecimalFootToFoot(num1);
                            break;
                        case "f2df":
                            response = Conversions.FootToDecimalFoot(num1);
                            break;
                        case "d2fi":
                            response = Conversions.DecimalInchToFoot(num1);
                            break;
                        case "f2di":
                            response = Conversions.FootToDecimalInch(num1);
                            break;
                        default:
                            response = 0m;
                            break;
                    ...
                    //var response = (function == "f2df") ? Conversions.FootToDecimalFoot(Convert.ToDecimal(num1)) : Conversions.DecimalFootToFoot(Convert.ToDecimal(num1));
        ...
        private List<decimal> UndoStackBuilder()
        {
            List<decimal> tempListForUndoRedo = new List<decimal>();

            foreach (var item in undoRedoStack.Peek())
            {
                tempListForUndoRedo.Add(item);
            }

            return tempListForUndoRedo;
        }
